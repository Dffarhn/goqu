...
**⚠️ PENTING - Filtering Leaf Accounts untuk Tampilan:**

API mengembalikan **semua accounts** (baik group maupun detail). Untuk tampilan di frontend (dropdown, form selection, dll.), **WAJIB filter hanya detail accounts (leaf)** dengan kondisi:
- `isGroup === false` 
- `isActive === true`

**Gunakan detail accounts untuk:**
- Dropdown pemilihan akun di form jurnal
- Dropdown pemilihan akun di buku besar
- Form create/edit jurnal entry
- Semua form yang memerlukan pemilihan akun spesifik

**Gunakan semua accounts (termasuk group) untuk:**
- Tampilan tree COA (untuk navigasi hierarkis)
- Laporan keuangan (untuk grouping)
- Form create/edit COA (untuk memilih parent account)
...
### ⚠️ Filtering untuk Tampilan COA

**PENTING:** Untuk semua tampilan COA di form/dropdown, **HANYA gunakan detail accounts (leaf)**:

```javascript
// Helper function untuk mendapatkan hanya detail accounts
function getDetailAccounts(accounts) {
...
**Alasan:**
- Group accounts tidak bisa digunakan untuk transaksi jurnal
- Hanya detail accounts yang memiliki `restriction`, `report`, dan `category`
- User hanya perlu memilih akun yang bisa digunakan untuk transaksi
...
Saat memuat COA untuk form jurnal entry, **WAJIB filter hanya detail accounts**:
...
// Gunakan detailAccounts untuk dropdown pemilihan akun
```

Ini memastikan user hanya bisa memilih akun yang valid untuk transaksi.
...
### ✅ 5.5. Filter Leaf Accounts untuk Tampilan

**WAJIB:** Implementasikan filter untuk hanya menampilkan detail accounts di semua form/dropdown:

```typescript
// Helper function
function getDetailAccounts(accounts: Account[]): Account[] {
  return accounts.filter(
    (acc) => !acc.isGroup && acc.isActive
  );
}
...
- `src/pages/takmir/jurnal_page/jurnal_form_page.jsx` ✅ (sudah ada)
- `src/pages/takmir/jurnal_page/jurnal.jsx` ✅ (sudah ada)
- `src/pages/takmir/buku_besar_page/buku_besar.jsx` ✅ (sudah ada)
- Semua component lain yang menggunakan COA dropdown
...
- [ ] Filter COA untuk hanya menampilkan detail accounts (`isGroup: false`) di semua form/dropdown
- [ ] Validasi bahwa group accounts tidak muncul di dropdown pemilihan akun
- [ ] Test bahwa hanya detail accounts yang aktif yang ditampilkan
